...
{a. PosicionarYLeerNodo(A, nodo , NRR) es un procedimiento que se posiciona en un nodo hijo de la raiz para poder seguir buscando la 
clave buscada. La variable nodo corresponde al array de las claves correspondiente al nodo hijo, A es el arbol y NRR el indice.
NRR debe ser pasado por valor , mientras que NRR y A por referencia.}

{b. claveEncontrada() es un procedimiento que almacena en la variable clave_encontrada true o false, dependiendo si la variable clave
se encontro en el arbol , y si se encontro almacena en la posicion en la variable pos . La variable clave_encontrada debe ser pasada
por referencia ya que se debe cambiar su valor , al igual que pos , clave por valor porque solo se necesita su valor y no modificarlo
y nodo y A se deben pasar por referencia  }
